...
public class FileUploadController {
    ...
    public ResponseEntity<Map<String, String>> uploadPortfolioImage(@RequestParam("file") MultipartFile file) {
        try {
            String fileUrl = fileStorageService.storePortfolioImage(file);
            Map<String, String> response = new HashMap<>();
            response.put("url", fileUrl);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
    ...
    @GetMapping("/portfolio-images/{fileName:.+}")
    public ResponseEntity<Resource> getPortfolioImage(@PathVariable String fileName) {
        try {
            Path filePath = fileStorageService.getPortfolioImagePath(fileName);
            Resource resource = new UrlResource(filePath.toUri());
    ...
    public ResponseEntity<Resource> getCompanyLogo(@PathVariable String fileName) {
        try {
            Path filePath = fileStorageService.getCompanyLogoPath(fileName);
            Resource resource = new UrlResource(filePath.toUri());
